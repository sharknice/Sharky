namespace Sharky.MicroTasks.Protoss
{
    public class PhoenixSubAttackTask : AttackSubTask
    {
        ...
        PhoenixGroupMicroController PhoenixGroupMicroController;

        private int groupSize;

        public bool AllSupport { get; set; }
        public bool AlwaysAttack { get; set; }
        public bool AllAttack { get; set; }
        ...
        /// <summary>
        /// Units estimated to be killed by phoenix group, enemy units that die within 7 range of any phoenix in task
        /// </summary>
        ...
        public PhoenixSubAttackTask(DefaultSharkyBot defaultSharkyBot, IAttackTask parentTask, PhoenixGroupMicroController phoenixMicroController, int minimumAttackSize,
            float priority, bool enabled = false, bool alwaysAttack = true)
        {
            ...
            PhoenixGroupMicroController = phoenixMicroController;
            ...
            SwitchRolesAfterEnemyStaticDefenseEstablished = false;
        }
        ...
        {
            if ((AllSupport || group.GroupRole == GroupRole.Support) && group.Commanders.Any())
            {
                var vector = group.Commanders.FirstOrDefault().UnitCalculation.Position;
                var nearbyUnit = ActiveUnitData.Commanders.Values.Where(u => !u.UnitCalculation.Unit.IsHallucination && u.UnitCalculation.Unit.UnitType != (uint)UnitTypes.PROTOSS_OBSERVER && u.UnitCalculation.Unit.UnitType != (uint)UnitTypes.PROTOSS_ORACLE && u.UnitCalculation.Unit.UnitType != (uint)UnitTypes.PROTOSS_DARKTEMPLAR && u.UnitCalculation.NearbyEnemies.Any(e => e.Unit.UnitType != (uint)UnitTypes.PROTOSS_ARCHON) && !UnitCommanders.Contains(u)).OrderBy(u => Vector2.DistanceSquared(vector, u.UnitCalculation.Position)).FirstOrDefault();
                if (nearbyUnit != null)
                {
                    var position = nearbyUnit.UnitCalculation.NearbyEnemies.FirstOrDefault(e => e.Unit.UnitType != (uint)UnitTypes.PROTOSS_ARCHON).Position;
        ...
        void UpdateStates()
        {
            ...
            foreach (var group in Groups)
            {
                ...
                {
                    if (group.Commanders.All(c => c.UnitCalculation.Unit.Shield > c.UnitCalculation.Unit.ShieldMax * .9f && c.UnitCalculation.Unit.Energy > 40))
                    {
                        foreach (var commander in group.Commanders)
                        {
                            commander.UnitRole = UnitRole.Attack;
                ...
                else if (group.Commanders.Sum(c => c.UnitCalculation.Unit.Shield) < group.Commanders.Sum(c => c.UnitCalculation.Unit.ShieldMax) * .5f || group.Commanders.All(c => c.UnitCalculation.Unit.Energy <= 40 && !c.UnitCalculation.NearbyEnemies.Any(e => e.Unit.BuffIds.Contains((uint)Buffs.GRAVITONBEAM))))
                {
                    ReportResults();
                ...
                else
                {
                    ...
                    if (leader == null)
                    {
                        leader = group.Commanders.Where(c => c.UnitCalculation.Unit.Shield > c.UnitCalculation.Unit.ShieldMax * .5f).OrderBy(c => Vector2.DistanceSquared(c.UnitCalculation.Position, groupVector)).FirstOrDefault();
                        if (leader != null)
                        {
                    ...
                    foreach (var commander in group.Commanders)
                    {
                        if (commander.UnitCalculation.Unit.Shield <= commander.UnitCalculation.Unit.ShieldMax * .5f)
                        {
                            commander.UnitRole = UnitRole.Regenerate;
                            commander.UnitCalculation.TargetPriorityCalculation.TargetPriority = TargetPriority.FullRetreat;
                        }
                        else if ((commander.UnitRole == UnitRole.Regenerate || commander.UnitRole == UnitRole.Hide) && commander.UnitCalculation.Unit.Shield > commander.UnitCalculation.Unit.ShieldMax * .9f && commander.UnitCalculation.Unit.Energy > 40)
                        {
        ...
        {
            Console.WriteLine($"{FrameToTimeConverter.GetTime(MacroData.Frame)} Phoenix Report: Deaths:{Deaths}, Kills:{Kills.Count()}");
            ...
            foreach (var group in Groups)
            {
                ...
                if (group.Commanders.FirstOrDefault().UnitCalculation.TargetPriorityCalculation.AirWinnability > .5f)
                {
                    if (group.Commanders.Count(c => c.UnitRole == UnitRole.Attack || c.UnitRole == UnitRole.Leader) >= MinimumAttackSize)
                    {
                        ...
                        {
                            actions.AddRange(PhoenixGroupMicroController.Attack(group.Commanders, groupAttackPoint, retreatSpot, groupCenter, frame, true));
                        }
                        else
                        {
                            actions.AddRange(PhoenixGroupMicroController.Attack(group.Commanders, groupAttackPoint, retreatSpot, groupCenter, frame, false));
                        }
                    }
                    else
                    {
                        actions.AddRange(PhoenixGroupMicroController.Retreat(group.Commanders, retreatSpot, groupCenter, frame));
                    }
                }
                else
                {
        ...
        Point2D GetRegenerationSpot(MicroGroup group, Point2D defensePoint, Point2D armyPoint, int frame)
        {
            ...
            if (group.Commanders.FirstOrDefault().UnitRole == UnitRole.Hide || armyPoint == null)
            {
        ...
        public override IEnumerable<SC2APIProtocol.Action> Retreat(Point2D defensePoint, Point2D armyPoint, int frame)
        {
            ...
            foreach (var group in Groups)
            {
                ...
                var groupCenter = TargetingService.GetArmyPoint(group.Commanders);
                actions.AddRange(PhoenixGroupMicroController.Retreat(group.Commanders, retreatPoint, groupCenter, frame));
            }
        ...
        public override IEnumerable<SC2APIProtocol.Action> Support(IEnumerable<UnitCommander> mainUnits, Point2D attackPoint, Point2D defensivePoint, Point2D armyPoint, int frame)
        {
            if (AlwaysAttack)
            {
                return Attack(attackPoint, defensivePoint, armyPoint, frame);
            }
            ...
            {
                var groupCenter = TargetingService.GetArmyPoint(group.Commanders);
                actions.AddRange(PhoenixGroupMicroController.Support(group.Commanders, mainUnits, attackPoint, defensivePoint, groupCenter, frame));
            }
        ...
        public override IEnumerable<SC2APIProtocol.Action> SupportRetreat(IEnumerable<UnitCommander> mainUnits, Point2D attackPoint, Point2D defensivePoint, Point2D armyPoint, int frame)
        {
            ...
            {
                var groupCenter = TargetingService.GetArmyPoint(group.Commanders);
                actions.AddRange(PhoenixGroupMicroController.Support(group.Commanders, mainUnits, defensivePoint, defensivePoint, groupCenter, frame));
            }
        ...
        public override IEnumerable<SC2APIProtocol.Action> SplitArmy(int frame, IEnumerable<UnitCalculation> closerEnemies, Point2D attackPoint, Point2D defensePoint, Point2D armyPoint)
        {
            ...
            foreach (var group in Groups)
            {
                ...
                if (leader == null)
                {
                    leader = group.Commanders.OrderBy(c => Vector2.DistanceSquared(c.UnitCalculation.Position, groupVector)).FirstOrDefault();
                    if (leader != null)
                    {
                        leader.UnitRole = UnitRole.Leader;
                    }
                }
                foreach (var commander in group.Commanders.Where(c => c.UnitRole != UnitRole.Leader))
                {
                    commander.UnitRole = UnitRole.Attack;
                }

                var groupAttackPoint = defensePoint;
                var retreatSpot = GetRegenerationSpot(group, defensePoint, armyPoint, frame);
