namespace Sharky.MicroTasks
{
    public class DefensiveStalkerZealotWarpInTask : MicroTask
    {
        ...
        public override IEnumerable<SC2Action> PerformActions(int frame)
        {
            ...
            {
                if (pylon.NearbyEnemies.Any(e => e.FrameLastSeen == frame && e.UnitClassifications.HasFlag(UnitClassification.ArmyUnit) && !e.Unit.IsHallucination && e.Unit.UnitType != (uint)UnitTypes.ZERG_CHANGELING && e.Unit.UnitType != (uint)UnitTypes.ZERG_CHANGELINGZEALOT))
                {
                    if (pylon.TargetPriorityCalculation.GroundWinnability < 1 || !pylon.NearbyAllies.Any(a => a.UnitClassifications.HasFlag(UnitClassification.ArmyUnit)))
                    {
                        ...
                        if (location != null)
                        {
                            ...
                            if (!pylon.NearbyEnemies.Any(e => e.Unit.UnitType == (uint)UnitTypes.TERRAN_HELLION || e.Unit.UnitType == (uint)UnitTypes.TERRAN_HELLIONTANK || e.Unit.UnitType == (uint)UnitTypes.TERRAN_REAPER) && pylon.NearbyEnemies.Any(e => !e.Unit.IsFlying))
                            {
