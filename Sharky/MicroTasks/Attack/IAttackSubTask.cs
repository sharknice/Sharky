namespace Sharky.MicroTasks.Attack
{
    public interface IAttackSubTask : IMicroTask
    {
        ...
        IEnumerable<SC2Action> Defend(Point2D target, Point2D defensePoint, Point2D armyPoint, int frame);
        IEnumerable<SC2Action> Contain(Point2D attackPoint, Point2D defensePoint, Point2D armyPoint, int frame);
