...
namespace Sharky.MicroTasks
{
    public class HallucinationScoutTask : MicroTask
    {
        ...
        public HallucinationScoutTask(TargetingData targetingData, BaseData baseData, bool enabled, float priority, IIndividualMicroController individualMicroController)
        {
            TargetingData = targetingData;
            BaseData = baseData;
            Priority = priority;
        ...
        public override IEnumerable<SC2APIProtocol.Action> PerformActions(int frame)
        {
            ...
            else
            {
                foreach (var commander in UnitCommanders)
                {
                    ...
                    {
                        if (Vector2.DistanceSquared(new Vector2(ScoutLocations[ScoutLocationIndex].X, ScoutLocations[ScoutLocationIndex].Y), new Vector2(commander.UnitCalculation.Unit.Pos.X, commander.UnitCalculation.Unit.Pos.Y)) < 2)
                        {
                            ScoutLocationIndex++;
