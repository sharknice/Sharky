namespace Sharky.MicroTasks.Terran
{
    public class BansheeSubAttackTask : AttackSubTask
    {
        ...
        BansheeGroupMicroController BansheeMicroController;
        ...
        /// <summary>
        /// Units estimated to be killed by mutalisk group, enemy units that die within 7 range of any banshees in task
        /// </summary>
        ...
        public BansheeSubAttackTask(DefaultSharkyBot defaultSharkyBot, IAttackTask parentTask, BansheeGroupMicroController bansheeMicroController, int minimumAttackSize,
            float priority, bool enabled = false, bool alwaysAttack = true)
        {
            ...
            BansheeMicroController = bansheeMicroController;
        ...
        void ClaimCommanders(IEnumerable<UnitCommander> commanders)
        {
            foreach (var commander in commanders)
            {
                ...
                {
                    if (commander.UnitCalculation.Unit.UnitType == (uint)UnitTypes.TERRAN_BANSHEE)
                    {
        ...
        void UpdateStates()
        {
            if (SwitchRolesAfterEnemyStaticDefenseEstablished && Deaths > 0 && Groups.Any(g => g.GroupRole != GroupRole.Support) && UnitCountService.EnemyCompleted(UnitTypes.TERRAN_MISSILETURRET) + UnitCountService.EnemyCompleted(UnitTypes.ZERG_SPORECRAWLER) + UnitCountService.EnemyCompleted(UnitTypes.PROTOSS_PHOTONCANNON) > 1)
            {
                foreach (var group in Groups)
                {
            ...
            foreach (var group in Groups)
            {
                ...
                else if (group.Commanders.All(c => c.UnitRole == UnitRole.Repair))
                {
                    if (group.Commanders.All(c => c.UnitCalculation.Unit.Health >= c.UnitCalculation.Unit.HealthMax))
                    {
                        foreach (var commander in group.Commanders)
                        {
                            commander.UnitRole = UnitRole.Attack;
                ...
                else
                {
                    ...
                    foreach (var commander in group.Commanders)
                    {
                        ...
                        else if ((commander.UnitRole == UnitRole.Repair || commander.UnitRole == UnitRole.Hide) && commander.UnitCalculation.Unit.Health > commander.UnitCalculation.Unit.HealthMax * .9f)
                        {
        ...
        {
            Console.WriteLine($"{FrameToTimeConverter.GetTime(MacroData.Frame)} Banshee Report: Deaths:{Deaths}, Kills:{Kills.Count()}");
        ...
        public override IEnumerable<SC2APIProtocol.Action> Attack(Point2D attackPoint, Point2D defensePoint, Point2D armyPoint, int frame)
        {
            ...
            {   
                ...
                if (group.Commanders.FirstOrDefault().UnitCalculation.TargetPriorityCalculation.AirWinnability > .5f || CloakedAndUndetected(group.Commanders.FirstOrDefault()))
                {
                    if (group.Commanders.Count(c => c.UnitRole == UnitRole.Attack || c.UnitRole == UnitRole.Leader) >= MinimumAttackSize)
                    {
                        ...
                        {
                            actions.AddRange(BansheeMicroController.Attack(group.Commanders, groupAttackPoint, retreatSpot, groupCenter, frame, true));
                        }
                        else
                        {
                            actions.AddRange(BansheeMicroController.Attack(group.Commanders, groupAttackPoint, retreatSpot, groupCenter, frame, false));
                        }
                    }
                    else
                    {
                        actions.AddRange(BansheeMicroController.Retreat(group.Commanders, retreatSpot, groupCenter, frame));
                    }
                }
                else
                {
        ...
        Point2D GetRegenerationSpot(MicroGroup group, Point2D defensePoint, Point2D armyPoint, int frame)
        {
            ...
            {
                var closestSafeBase = BaseData.SelfBases.OrderBy(b => Vector2.DistanceSquared(armyPoint.ToVector2(), b.Location.ToVector2())).FirstOrDefault(b => b.ResourceCenter != null && b.ResourceCenter.BuildProgress == 1 && !MapDataService.InEnemyVision(b.Location));
        ...
        public override IEnumerable<SC2APIProtocol.Action> Retreat(Point2D defensePoint, Point2D armyPoint, int frame)
        {
            ...
            foreach (var group in Groups)
            {
                ...
                var groupCenter = TargetingService.GetArmyPoint(group.Commanders);
                actions.AddRange(BansheeMicroController.Retreat(group.Commanders, retreatPoint, groupCenter, frame));
            }
        ...
        public override IEnumerable<SC2APIProtocol.Action> Support(IEnumerable<UnitCommander> mainUnits, Point2D attackPoint, Point2D defensivePoint, Point2D armyPoint, int frame)
        {
            ...
            {
                var groupCenter = TargetingService.GetArmyPoint(group.Commanders);
                actions.AddRange(BansheeMicroController.Support(group.Commanders, mainUnits, attackPoint, defensivePoint, groupCenter, frame));
            }
        ...
        public override IEnumerable<SC2APIProtocol.Action> SupportRetreat(IEnumerable<UnitCommander> mainUnits, Point2D attackPoint, Point2D defensivePoint, Point2D armyPoint, int frame)
        {
            ...
            {
                var groupCenter = TargetingService.GetArmyPoint(group.Commanders);
                actions.AddRange(BansheeMicroController.Support(group.Commanders, mainUnits, defensivePoint, defensivePoint, groupCenter, frame));
            }
