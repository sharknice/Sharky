...
{
    public class BansheeInGroupMicroController : BansheeMicroController
    {
        float KiteDistance = 2.625f; // 3.85 (+1.4) movement speed / 2 = 2.625 
        float HalfWeaponCooldown = 9f; // frames until half cooldown (SharkyOptions.FramesPerSecond * 0.89f) / 2f) = 9.968 frames


        public BansheeInGroupMicroController(DefaultSharkyBot defaultSharkyBot, IPathFinder sharkyPathFinder, MicroPriority microPriority, bool groupUpEnabled)
        ...
        {
            var range = KiteDistance + commander.UnitCalculation.Range + commander.UnitCalculation.Unit.Radius + bestTarget.Unit.Radius;
        ...
        public override bool AttackBestTarget(UnitCommander commander, Point2D target, Point2D defensivePoint, Point2D groupCenter, UnitCalculation bestTarget, int frame, out List<SC2APIProtocol.Action> action)
        {
            ...
            {
                var enemyPosition = GetBestTargetAttackPoint(commander, bestTarget);
                if (commander.UnitCalculation.NearbyEnemies.Any(e => AvoidedUnitTypes.Contains((UnitTypes)e.Unit.UnitType)))
                {
                    action = commander.Order(frame, Abilities.MOVE, enemyPosition);
                    return true;
                }
                ...
                {
                    if (bestTarget.Unit.Tag == commander.UnitCalculation.Unit.Tag || !commander.UnitCalculation.NearbyEnemies.Any(e => bestTarget.Unit.Tag == e.Unit.Tag))
                    {
                        action = commander.Order(frame, Abilities.MOVE, enemyPosition);
                    }
                    else
        ...
        public override List<SC2APIProtocol.Action> Idle(UnitCommander commander, Point2D defensivePoint, int frame)
        {
            ...
            {
                var closestScv = commander.UnitCalculation.NearbyAllies.Where(a => a.Unit.UnitType == (uint)UnitTypes.TERRAN_SCV).OrderBy(a => Vector2.DistanceSquared(a.Position, commander.UnitCalculation.Position)).FirstOrDefault();
                if (closestScv != null)
                {
                    return commander.Order(frame, Abilities.MOVE, closestScv.Unit.Pos.ToPoint2D());
                }
            }
            return action;
        }
        ...
        {
            if (commander.UnitCalculation.NearbyEnemies.Any(e => e.UnitClassifications.HasFlag(UnitClassification.Worker)) && (commander.UnitCalculation.TargetPriorityCalculation.TargetPriority == TargetPriority.KillWorkers || CloakedAndUndetected(commander)))
            {
        ...
        {
            if ((commander.UnitCalculation.TargetPriorityCalculation.TargetPriority == TargetPriority.KillWorkers || CloakedAndUndetected(commander)) && commander.UnitCalculation.NearbyEnemies.Count(e => e.UnitClassifications.HasFlag(UnitClassification.Worker)) > 1)
            {
            ...
            return GetBestTargetFromList(commander, commander.UnitCalculation.NearbyEnemies.Where(e => DamageService.CanDamage(commander.UnitCalculation, e) && bestTarget.Unit.Tag != e.Unit.Tag), null);
        }
    }
}
